...
class ConversationListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if user.role == 'doctor':
            ...
            patient = User.objects.filter(id=patient_id).first() if patient_id else None
            if not doctor or not patient:
                return Response({'error': 'Doctor profile or patient not found'}, status=status.HTTP_404_NOT_FOUND)
